...
**The description of new plants occurs at a fairly consistent rate, with a mean of `r tax.nov.year.mean` per year (1999 – 2017)**
...
Use [POWO](http://www.plantsoftheworldonline.org/) API to assign IPNI IDs to Red List plant names.
Then use IPNI ID to get date/year that plant name description was published
Compare assessment year with name description publication year
...
a.	Click dropdown next to the IPNI id, select “Edit column” ->  “Add column by fetching URLs”
b.	Build the URL by entering the following into the expression box:
'http://ipni.org/urn:lsid:ipni.org:names:'+value
c.	Give your new column a title (e.g. “IPNI LSID data”) and a throttle delay of 1000 milliseconds, click OK
...
a.	Click the dropdown next to the “IPNI LSID data” column, select “Edit column” -> “Add column based on this column”
b.	Select the year tag in the RDF XML and extract its contents: type in:
value.parseHtml().select("tn|year")[0].htmlText()
c.	Give your new column a title (‘publication year’) and click OK

Save as [all.plants.IPNI.csv](https://github.com/stevenpbachman/RedList_Challenges/blob/master/01_data/all.plants.IPNI.csv)
...
We need to get IPNI IDs for red list plant species and then match that with IPNI file so that we have year described and year on red list
